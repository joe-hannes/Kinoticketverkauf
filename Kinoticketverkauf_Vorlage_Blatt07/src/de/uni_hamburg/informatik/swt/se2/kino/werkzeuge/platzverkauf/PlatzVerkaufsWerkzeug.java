...
import de.uni_hamburg.informatik.swt.se2.kino.werkzeuge.bezahlung.BarZahlungsWerkzeug;
...
public class PlatzVerkaufsWerkzeug
{
    ...
    /**
     * Erzeugt ein neues BarZahlungsWerkzeug und startet die Barzahlung.
     * Es kann nur verkauft werden, wenn die Barzahlung den Preis der Plaetze erreicht oder überschreitet
     */
    private void fuehreBarzahlungDurch()
    {
        // TODO : Barzahlung durchführen und Plätze nur bei Erfolg verkaufen
        //Als Parameter den Preis aller Ausgewählten Plaetze übergeben. Der Preis wird mit der jeweiligen Anzahl an Plaetzen multipliziert
        _barZahlungsWerkzeug = new BarZahlungsWerkzeug(
                _vorstellung.getPreis() * _ui.getPlatzplan()
                    .getAusgewaehltePlaetze()
                    .size());

        //Beobachter hinzufügen - So kann über Aktionen des Ok-Buttons informiert
        _barZahlungsWerkzeug.registriereBeobachter(new SubwerkzeugObserver()
        {
            @Override
            public void reagiereAufAenderung()
            {
                _vorstellung.verkaufePlaetze(_ui.getPlatzplan()
                    .getAusgewaehltePlaetze());
            }
        });
        _barZahlungsWerkzeug.initialisiereUI();
